<template>
  <div id="topMenu" @click="gameListOn(true)">
    <h2 class="hidden">
      topMenu
    </h2>
    <div class="wrap">
      <img class="logo pc" src="@/assets/logo.png" alt="로고" />
      <img class="logo mobile" src="@/assets/logo.png" alt="로고" />
    </div>
  </div>
</template>
    ...
    window.addEventListener("scroll", async () => {
      ...
      if (window.pageYOffset <= 10) {
        ...
        if (w > 800) {
          ...
          gsap.to(topM, { background: "#fff", duration: 0.5 });
        ...
        if (w > 800) {
          gsap.to(topM, { height: "3rem", duration: 0.5 });
          gsap.to(topM, { background: "#eeeeee", duration: 0.5 });
